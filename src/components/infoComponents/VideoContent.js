...
const VideoContent = ({ infoTopics, deletePost, addPost }) => {
  ...
  return (
    <div className="VideoContent">
      ...
      <div className="float-right" role="addvideo">
        {user.admin && (
          <AddVideo
            infoTopics={infoTopics}
            setVideos={setVideoContent}
            addPost={addPost}
          />
        )}
      </div>
      <div className="w-50 d-flex justify-content-start">
        <div className="searchcontent" role="searchvideos">
          <Search
      ...
      <div className="searchmobile" role="searchvideos">
        <Search
          setSearchValue={setSearchFilter}
          searchValue={searchFilter}
          placeholderText={"Søk..."}
          ...
          <Card.Body>
            <Row>
              <Col md={5}>
                <div className="content">
                  {infoTopics
                    .filter(
                      (infoTopics) =>
                        infoTopics.id === filteredVideos.infoTopicId
                    )
                    .map((filteredTopics) => (
                      <p
                        role="sharedinfo"
                      ...
                      >
                        Delt {moment(filteredVideos.uploaded).calendar()} i{" "}
                  ...
                  <div hidden={!user.admin}>
                    <Button
                      variant="danger"
                      size="sm"
                      value={filteredVideos.id}
                      role="deletevideo"
                      onClick={() =>
                        deleteVideo(filteredVideos.id, filteredVideos.postId)
                      }
                    >
                      Slett
                    </Button>
              ...
              <Col md={7}>
                ...
                >
                  <Image
                    src={`https://img.youtube.com/vi/${filteredVideos.youTubeId}/0.jpg`}
                    alt={filteredVideos.title + " thumbnail"}
                  />
                </div>
              </Col>
            </Row>
        ...
        <Modal key={i} show={show === i} onHide={handleClose} centered>
          ...
          <Modal.Body>
            <iframe
              ...
              frameBorder="0"
              src={`https://www.youtube.com/embed/${filteredVideos.youTubeId}`}
              width="100%"
              height="300px"
            />
            <p>{filteredVideos.description}</p>
            <Link to={`/Forum/${filteredVideos.postId}`}>
              Diskuter i forumet
            </Link>
          </Modal.Body>
          <Modal.Footer>
