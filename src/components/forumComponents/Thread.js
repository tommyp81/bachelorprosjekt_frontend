...
const Thread = ( { subtopics, topics, users, history, updatePostInArray, deletePost }) => {
            ...
            <Card.Body>
              <div className="float-left">
                <p>Postet av <b>{users && users.length && users.find(u => u.id === post.userId)?.username}</b> {moment(post.date).calendar()}</p>
                  ...
                  {post.like_Count} <LikeButton id={postId} liked={liked} setLiked={setLiked} isPost={true} updatePostLike={updatePostLike}/> &nbsp;
              ...
              <div className="float-right" hidden={!(user.id === post.userId)}>
                ...
                <Button variant="danger" size="sm" onClick={deleteThread} value={post.id}>Slett</Button>
              </div>
